...
router.get("/", (req, res) => {
    ...
    const items = db.prepare(`SELECT * FROM community_posts ${where} ORDER BY datetime(created_at) DESC LIMIT ? OFFSET ?`).all(...vals);
...
router.patch("/:id", (req, res) => {
    const id = Number(req.params.id);
    const allowed = ["author_id", "title", "description", "image_url"];
...
router.get("/:id/comments", (req, res) => {
    ...
    const items = db.prepare(`SELECT * FROM comments WHERE post_type = 'community' AND post_id = ? ORDER BY datetime(created_at) ASC`).all(postId);
    res.json({ items });
});
