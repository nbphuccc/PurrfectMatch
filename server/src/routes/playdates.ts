...
router.get("/", (req, res) => {
  ...
  if (city) { whereParts.push("city = ?"); vals.push(city); }
  if (q) { whereParts.push("(title LIKE ? OR description LIKE ? OR dog_breed LIKE ?)"); vals.push(`%${q}%`, `%${q}%`, `%${q}%`); }
  const whereSql = whereParts.length ? `WHERE ${whereParts.join(" AND ")}` : "";

  const items = db.prepare(
    `SELECT * FROM playdate_posts ${whereSql} ORDER BY datetime(created_at) DESC LIMIT ? OFFSET ?`
...
router.post("/", (req, res) => {
  ...
  const {
    ...
    city, state, zip, when_at, place, image_url
  } = req.body;
  ...
  `).run(
    Number(author_id), String(title), String(description), String(dog_breed), String(address),
    String(city), String(state), String(zip), String(when_at), String(place),
    image_url ?? null, now
  );
